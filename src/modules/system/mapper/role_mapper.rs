...
//增删改查菜单与角色关联
rbatis::crud!(RoleMenuMerge {}, "mxx_system_role_menus_merge");

/// 查询角色和菜单是否已关联
#[py_sql("select count(*) from mxx_system_role_menus_merge where role_id = #{role_id} and menu_id = #{menu_id}")]
pub async fn find_merge_by_unique(rb: &RBatis, role_id: Option<u64>, menu_id: Option<u64>, ) -> rbatis::Result<u64> {
    impled!()
}

///角色id查询所有关联的菜单id
#[sql("select * from mxx_system_role_menus_merge where role_id = ?")]
pub async fn get_merge_by_role_id(rb: &RBatis,role_id: &Option<u64>,) -> rbatis::Result<Vec<RoleMenuMerge>> {
    impled!()
}

///查询当前菜单id关联的所有角色ID
#[sql("select role_id from mxx_system_role_menus_merge where menu_id = ?")]
