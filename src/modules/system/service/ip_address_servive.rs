...
pub async fn ip_address_save(mut item: &IpAddress) -> rbatis::Result<u64> {
    ...
    let ip_list = ip_address_mapper::select_ip_address_list(pool!(), &item.start_ip, &item.end_ip).await?;
...
pub async fn get_ip_address_by_id(id: u64) -> rbatis::Result<Option<IpAddress>> {
    let st = IpAddress::select_by_column(pool!(), "id", id).await?
        .into_iter()
        .next();
    return Ok(st);
}
...
pub async fn find_ip_address_by_ip(ip: &Option<u32>) -> rbatis::Result<Option<IpAddress>> {
    let st = ip_address_mapper::find_ip_address_by_ip(pool!(), ip).await?.into_iter().next();
    return Ok(st);
}
...
pub async fn ip_address_page(item: QueryPageRequest) -> rbatis::Result<Page<IpAddress>> {
    ...
    let list = IpAddress::select_page(pool!(), page_req, &ip_dto).await?;
    return Ok(list);
}