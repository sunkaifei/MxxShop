...
pub async fn save_dict_type(payload: DictTypeSaveRequest) -> Result<ExecResult> {
    ...
    Ok(DictType::insert(&RB.clone(), &data_req).await?)
}
...
pub async fn save_dict_data(payload: DictDataSaveRequest) -> Result<ExecResult> {
    ...
    data_req.id = generate_snowflake_id();
    Ok(DictData::insert(&RB.clone(), &data_req).await?)
}

///批量删除字典类型
pub async fn bath_delete_type(ids: Vec<Option<String>>) -> Result<u64> {
    let result = DictType::delete_in_column(&RB.clone(), "id", &ids).await;
    return Ok(result?.rows_affected);
}

///批量删除字典数据
pub async fn bath_delete_data(ids: Vec<Option<String>>) -> Result<u64> {
    let result = DictData::delete_in_column(&RB.clone(), "id", &ids).await;
    return Ok(result?.rows_affected);
}
...
pub async fn get_data_by_id(id: u64) -> rbatis::Result<Option<DictData>> {
    let st = DictData::select_by_column(&RB.clone(), "id", id).await?
        .into_iter()
        .next();
    Ok(st)
}
...
pub async fn get_dict_data_list(item: DictDataPageRequest) -> rbatis::Result<Page<DictData>> {
    ...
    let list = DictData::select_page(&mut RB.clone(), page_req, &page_query).await?;
    return Ok(list);
}
...
pub async fn get_dict_type_page(item: DictTypePageRequest) -> rbatis::Result<Page<DictType>> {
    ...
    let list = DictType::select_page(&mut RB.clone(), page_req, &page_query).await?;
    return Ok(list);
}

///按字典类型获取字典数据列表
pub async fn get_dict_type_list(dict_type: &String) -> rbatis::Result<Vec<DictData>> {
    let list = DictData::select_by_column(&mut RB.clone(), "dict_type", dict_type).await;
    return list;
}