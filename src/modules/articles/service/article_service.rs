...
pub async fn save_article(payload: ArticlesSaveRequest) -> Result<u64> {
    ...
    let unique_num = article_mapper::find_by_title_unique(&RB.clone(), &article_data.title, &article_data.user_id).await?;
    if unique_num > 0 {
    ...
    let rows = Articles::insert(&tx, &article_data).await;
    if rows.is_ok() {
        let jieba = Jieba::new();
...
pub async fn find_short_url_unique() -> Option<String> {
    ...
    for _ in 0..5 {
        ...
        let unique_num = article_mapper::find_by_short_url_unique(&RB.clone(), &short_url).await;
...
pub async fn delete_article(id: u64) -> Result<u64> {
    let mut tx = RB.acquire_begin().await?;
    let rows = Articles::delete_by_column(&RB.clone(), "id", id).await;
    tx.commit().await?;
    tx.rollback().await?;
    return Ok(rows.unwrap_or_default().rows_affected);
}
...
pub async fn delete_article_by_ids(ids: Vec<String>) -> Result<u64> {
    let mut tx = RB.acquire_begin().await?;
    let rows = Articles::delete_by_column(&tx, "id", ids).await;
    tx.commit().await?;
    tx.rollback().await?;
    return Ok(rows.unwrap_or_default().rows_affected);
}
...
pub async fn update_article(payload: ArticlesUpdateRequest) -> Result<u64> {
    ...
    let mut tx = RB.acquire_begin().await?;
    let rows = Articles::update_by_column(&RB.clone(), &article_data, "id").await;
    tx.commit().await?;
    tx.rollback().await?;
    return Ok(rows.unwrap_or_default().rows_affected);
}
...
pub async fn get_by_id(id: u64) -> rbatis::Result<Option<Articles>> {
    let st = Articles::select_by_column(&RB.clone(), "id", id).await?
        .into_iter().next();
    Ok(st)
}

pub async fn get_by_short_url(short_url: Option<String>) -> rbatis::Result<Option<Articles>> {
    let st = Articles::select_by_column(&RB.clone(), "short_url", short_url).await?
        .into_iter().next();
    Ok(st)
}
...
pub async fn select_list(mut item: ArticlesListRequest) -> rbatis::Result<Vec<Articles>> {
    ...
    let list = Articles::select_list(&mut RB.clone(), &item).await;
    return list;
}

pub async fn get_by_page(item: ArticlesPageRequest) -> rbatis::Result<Page<Articles>> {
    let page_req = &PageRequest::new(item.page_no.clone(), item.page_size.clone());
    let list = Articles::select_page(&mut RB.clone(), page_req, &item).await;
    return list;
}
